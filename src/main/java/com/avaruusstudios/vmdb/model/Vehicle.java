 ...
 * physical attributes (make, model, year, capacity), lease terms, active status, and financial discount.
 * </p>
 *
 * <p>
 * Each vehicle is uniquely identified by its {@code VehicleID} and {@code VehicleNumber}.
 * Its properties are designed to support data binding with JavaFX UI components, making it suitable
 * for a responsive desktop application. This class directly maps to the `Vehicles` table in the SQLite database
 * as per the finalized schema.
 * </p>
 *
 * @see Transaction
 */
public class Vehicle {
     ...
     * This field is **required** and must be unique across all vehicles
     * (corresponds to {@code VehicleNumber TEXT NOT NULL} in the database).
     */
     ...
     * This field is **required** (corresponds to {@code IsActive INTEGER NOT NULL} in the database, 0 for false, 1 for true).
     */
     ...
     * are initialized to sensible defaults to ensure valid state.
     ...
     */
    public Vehicle() {
        // Initialize immutable ID property first, as it's set once by the DB
        this.vehicleID = new SimpleObjectProperty<>(this, "vehicleID", null);

        // Initialize mutable properties to their default JavaFX Property types
        ...
        // Set default values for mandatory fields to ensure a valid object state
        setVehicleNumber(""); // Will be validated by setter
        setCapacity(1);     // Smallest valid capacity
        setLeaseStartDate(LocalDate.now());
        setIsActive(true);
    ...
    public Vehicle(Integer vehicleID, String vehicleNumber, String make, String model, Integer year, int capacity,
                   LocalDate leaseStartDate, LocalDate leaseEndDate, boolean isActive, BigDecimal discount, String notes) {
    ...
    /**
     * Sets the unique ID for this vehicle. This method is designed to be package-private
     ...
     */
    void _setVehicleID(Integer id) { // Package-private for DAO use only
    ...
    public void setVehicleNumber(String vehicleNumber) {
        String trimmedNumber = (vehicleNumber == null) ? null : vehicleNumber.trim();
     ...
     * <p>
     * The year must be a positive value. It cannot be before the year 2000 and cannot be in the future
     * relative to the current year (allowing current year + 1 for new models).
     * This field is optional. If setting to {@code null} or 0, ensure the application logic handles it correctly
     * as an unknown or unset year.
     * </p>
     *
     * @param year The manufacturing year to set. Can be {@code null} or 0, but if a positive value,
     * must not be before 2000 and not in the future.
     *
     ...
     * </p>
     * <p>
     * The format includes the vehicle ID, vehicle number, make, model, and year.
     * </p>
     *
     * @return A string in the format:
     * "Vehicle{ID=..., VehicleNumber=..., Make=..., Model=..., Year=...}"
     */
    @Override
